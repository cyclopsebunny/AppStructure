...
export function getCacheDebugInfo() {
  const loc = _cachedFacilityDoc?.canvasLocations?.[0];
  return {
    mounted:       _liveDebug.mounted,
    liveBuildings: _liveDebug.buildings,
    liveRows:      _liveDebug.rows,
    liveHandle:    _liveDebug.hasHandle,
    cacheHasDoc:   _cachedFacilityDoc !== null,
    cacheBuildings: loc?.buildings?.length ?? _cachedFacilityDoc?.buildings?.length ?? 0,
    cacheRows:      loc?.rows?.length      ?? _cachedFacilityDoc?.rows?.length      ?? 0,
    cacheHandle:   _cachedFacilityHandle !== null,
  };
}
...
export function FacilityCanvas() {
  ...
  // synthetic unmount (which fires before the async load resolves).
  const _isRestoringRef = useRef(false);

  // Refs that always hold the latest values so the unmount cleanup avoids stale closures.
  // Assignments happen below, after all useState calls that these reference.
  const _getDocPayloadRef = useRef<((mode: AppMode) => FacilityDocument) | null>(null);
  const _appModeRef = useRef<AppMode>('build');
  const _handleRef = useRef<FileSystemFileHandleLike | null>(null);
  ...
  // ── Canvas state persistence across route navigations ────────────────────────
  // Keep persistence refs current every render (assignments must be after all useState calls).
  _appModeRef.current = appMode;
  _handleRef.current = documentHandle;
  // Update live debug snapshot every render so AppShell's debug panel reflects current state.
  _liveDebug = { mounted: true, buildings: buildings.length, rows: rows.length, hasHandle: documentHandle !== null };

  // eslint-disable-next-line react-hooks/exhaustive-deps
  useEffect(() => {
    // Restore cached state from a previous visit to this route.
    if (_cachedFacilityDoc !== null) {
      // Mark restoration in-flight so the StrictMode synthetic cleanup (which fires
      // synchronously before loadDocumentFile resolves) does not overwrite the cache.
      // The flag resets via .finally() once the async load settles.
      _isRestoringRef.current = true;
      const docJson = JSON.stringify(_cachedFacilityDoc);
      const syntheticFile = new File([docJson], 'cached.json', { type: 'application/json' });
      void loadDocumentFile(syntheticFile).finally(() => {
  ...
  // Keep the ref current now that getDocumentPayload is in scope.
  _getDocPayloadRef.current = getDocumentPayload;
    ...
    if (pickerWindow.showSaveFilePicker) {
      ...
      try {
        handle = await pickerWindow.showSaveFilePicker({
          ...
          suggestedName: 'facility-layout.json',
          types: [
            {
      ...
      try {
        ...
        console.error('[FacilityCanvas] writeDocumentToHandle failed, falling back to download:', err);
  ...
  const handleOpenDocument = async () => {
    ...
    if (pickerWindow.showOpenFilePicker) {
      try {
        ...
        // Request write permission now so "Save" can overwrite later without a dialog.
  ...
  const loadDocumentFile = async (file: File | null) => {
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error('[FacilityCanvas] loadDocumentFile error:', err);
  ...
  return (
    <main className="facility-app">
      <section className={['facility-shell', appMode !== 'operations' ? 'facility-shell--has-panel' : ''].filter(Boolean).join(' ')}>
        ...
        {appMode !== 'operations' && <aside className="details-panel">
